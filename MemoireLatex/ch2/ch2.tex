...
To Design a process systems, there is so many framework to use, in this chapter will introduce one framework to modeling a process system.
\section{PGraph}
...
Considering the process-network the output materials of each operating unit are the inputs of different operating units.
...
which is the output of one or more operating units.
...
The Table \ref{tab:Petri Net and PNS} represent the different, and the common features, between these two mathematical tools, because the Petri is widely use more then PNS.
\begin {table}[H] 
\begin{tabular}{cc}
