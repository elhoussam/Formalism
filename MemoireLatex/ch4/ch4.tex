...
\paragraph{\emph{ a )~Condition : } } For the application of this rule, it must be verified that the partition To be transformed is not treated before
\vspace{1cm}
\begin{figure}[th]
	\centering %[scale=1.2]
...
\end{figure} 
In the first line of the previous action we import external file 'MyFunction.py', this file contain the functions to add new tag (material, product, operatingunit) to the xml file.
...
\paragraph{\emph{2)~VisitEdge : Transform Edge between two entitie into XML code :} } in this transformation we want to create link between nodes Material and Operating unit 
...
\paragraph{\emph{ a )~Condition } } it is the same with figure \ref{fig:condition of Raw Material for Transfromation} but here we select the edges
