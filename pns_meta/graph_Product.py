...
class graph_Product(graphEntity):
    ...
    def DrawObject(self, drawing, showGG = 0):
        ...
        self.image_gf22 = PhotoImage(format='gif',data=self.imageDict['p.gif' ])
        h = drawing.create_image(self.translate([37.0, 25.0]), tags = self.tag, image = self.image_gf22)
        self.gf22 = GraphicalForm(drawing, h, 'gf22', 'p.gif')
        self.graphForms.append(self.gf22)

        h = drawing.create_oval(self.translate([38.0, 50.0, 38.0, 50.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([17.0, 41.0, 17.0, 41.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([58.0, 40.0, 58.0, 40.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([35.0, 2.0, 35.0, 2.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([16.0, 13.0, 16.0, 13.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([58.0, 10.0, 58.0, 10.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        if self.semanticObject: drawText = self.semanticObject.Name.toString()
        else: drawText = "<Name>"
        font = tkFont.Font( family='Helvetica', size=12, weight='normal', slant='roman', underline=0)
        h = drawing.create_text(self.translate([35.0, 26.0, 35.0, 17.0])[:2], tags = self.tag, font=font, fill = 'black', anchor = 'center', text = drawText, width = '0', justify= 'left', stipple='' )
