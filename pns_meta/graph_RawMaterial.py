...
class graph_RawMaterial(graphEntity):
    ...
    def DrawObject(self, drawing, showGG = 0):
        ...
        self.image_gf17 = PhotoImage(format='gif',data=self.imageDict['d.gif' ])
        h = drawing.create_image(self.translate([35.0, 26.0]), tags = self.tag, image = self.image_gf17)
        self.gf17 = GraphicalForm(drawing, h, 'gf17', 'd.gif')
        self.graphForms.append(self.gf17)

        h = drawing.create_oval(self.translate([36.0, 50.0, 36.0, 50.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([15.0, 40.0, 15.0, 40.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([55.0, 41.0, 55.0, 41.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        if self.semanticObject: drawText = self.semanticObject.Name.toString()
        else: drawText = "<Name>"
        font = tkFont.Font( family='Helvetica', size=12, weight='normal', slant='roman', underline=0)
        h = drawing.create_text(self.translate([35.0, 31.0, 35.0, -2.0])[:2], tags = self.tag, font=font, fill = 'black', anchor = 'center', text = drawText, width = '0', justify= 'left', stipple='' )
        self.attr_display["Name"] = h
        self.gf18 = GraphicalForm(drawing, h, 'gf18', fontObject=font)
        self.graphForms.append(self.gf18)
