...
class graph_metarial(graphEntity):
    ...
    def DrawObject(self, drawing, showGG = 0):
        ...
        self.image_gf11 = PhotoImage(format='gif',data=self.imageDict['m.gif' ])
        h = drawing.create_image(self.translate([25.0, 25.0]), tags = self.tag, image = self.image_gf11)
        self.gf11 = GraphicalForm(drawing, h, 'gf11', 'm.gif')
        self.graphForms.append(self.gf11)

        h = drawing.create_oval(self.translate([24.0, 49.0, 24.0, 49.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([6.0, 42.0, 6.0, 42.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([41.0, 42.0, 41.0, 42.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([46.0, 10.0, 46.0, 10.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([26.0, -2.0, 26.0, -2.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        h = drawing.create_oval(self.translate([10.0, 5.0, 10.0, 5.0]), tags = (self.tag, 'connector'), outline = '', fill = '' )
        self.connectors.append( h )

        if self.semanticObject: drawText = self.semanticObject.Name.toString()
        else: drawText = "<Name>"
        font = tkFont.Font( family='Helvetica', size=12, weight='normal', slant='roman', underline=0)
        h = drawing.create_text(self.translate([26.0, 25.0, 26.0, 9.0])[:2], tags = self.tag, font=font, fill = 'black', anchor = 'center', text = drawText, width = '0', justify= 'left', stipple='' )
