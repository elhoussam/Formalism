"""
__omacsEx2_MDL.py_____________________________________________________

Automatically generated AToM3 Model File (Do not modify directly)
Author: sam
Modified: Fri May 12 17:05:32 2017
...
def omacsEx2_MDL(self, rootNode, omacsRootNode=None):
    ...
    # rate
    self.obj37.rate.setValue(0.11)

    self.obj37.graphClass_= graph_achieve
    if self.genGraphics:
       new_obj = graph_achieve(84.8262697098,362.145172582,self.obj37)
       new_obj.DrawObject(self.UMLmodel)
    ...
    # rate
    self.obj38.rate.setValue(0.22)

    self.obj38.graphClass_= graph_achieve
    if self.genGraphics:
       new_obj = graph_achieve(317.182310383,337.057717493,self.obj38)
       new_obj.DrawObject(self.UMLmodel)
       self.UMLmodel.addtag_withtag("achieve", new_obj.tag)
       new_obj.layConstraints = dict() # Graphical Layout Constraints 
       new_obj.layConstraints['Text Scale'] = 0.92
    ...
    # rate
    self.obj39.rate.setValue(0.12)

    self.obj39.graphClass_= graph_achieve
    if self.genGraphics:
       new_obj = graph_achieve(222.137563265,342.387775475,self.obj39)
       new_obj.DrawObject(self.UMLmodel)
       self.UMLmodel.addtag_withtag("achieve", new_obj.tag)
       new_obj.layConstraints = dict() # Graphical Layout Constraints 
    ...
    self.obj40=require(self)
    self.obj40.isGraphObjectVisual = True

    if(hasattr(self.obj40, '_setHierarchicalLink')):
      self.obj40._setHierarchicalLink(False)

    self.obj40.graphClass_= graph_require
    if self.genGraphics:
       new_obj = graph_require(340.414318872,195.904654541,self.obj40)
       new_obj.DrawObject(self.UMLmodel)
    ...
    self.obj41=require(self)
    self.obj41.isGraphObjectVisual = True

    if(hasattr(self.obj41, '_setHierarchicalLink')):
      self.obj41._setHierarchicalLink(False)

    self.obj41.graphClass_= graph_require
    if self.genGraphics:
       new_obj = graph_require(244.92103503,220.28528088,self.obj41)
       new_obj.DrawObject(self.UMLmodel)
       self.UMLmodel.addtag_withtag("require", new_obj.tag)
       new_obj.layConstraints = dict() # Graphical Layout Constraints 
    else: new_obj = None
    ...
    if self.genGraphics:
       new_obj = graph_require(153.01081005,221.905331689,self.obj42)
       new_obj.DrawObject(self.UMLmodel)
       self.UMLmodel.addtag_withtag("require", new_obj.tag)
       new_obj.layConstraints = dict() # Graphical Layout Constraints 
    else: new_obj = None
    ...
    if self.genGraphics:
       new_obj = graph_require(67.381555298,233.108740168,self.obj43)
       new_obj.DrawObject(self.UMLmodel)
       self.UMLmodel.addtag_withtag("require", new_obj.tag)
       new_obj.layConstraints = dict() # Graphical Layout Constraints 
    else: new_obj = None
...
(self.obj25,self.obj36,[295.70668967756524, 35.75, 326.11631962078917, 51.33360035309086, 349.56055185802165, 74.09238257128929],"true", 3) )
    ...
    self.drawConnections(
(self.obj29,self.obj42,[128.5783805680038, 285.70499998567504, 136.43730545936495, 251.3528148901846, 153.01081004957175, 221.9053316890469],"true", 3),
(self.obj29,self.obj43,[128.5783805680038, 285.70499998567504, 94.27615044002344, 262.7656996317734, 67.38155529802249, 233.10874016814495],"true", 3),
(self.obj29,self.obj37,[128.5783805680038, 308.20499998567504, 110.09154619972537, 338.85111411300124, 84.82626970983395, 362.1451725819457],"true", 3),
(self.obj29,self.obj39,[128.5783805680038, 308.20499998567504, 177.53664924416333, 320.79601384542224, 222.13756326537128, 342.3877754747146],"true", 3) )
    # Connections for obj30 (graphObject_: Obj6) named R2
    self.drawConnections(
(self.obj30,self.obj40,[307.3827052922357, 256.9737669028152, 319.8890905381401, 223.4518140486341, 340.414318871705, 195.90465454112874],"true", 3),
(self.obj30,self.obj41,[307.3827052922357, 256.9737669028152, 273.0486116211594, 242.54995581082727, 244.92103503030822, 220.28528088040454],"true", 3),
(self.obj30,self.obj38,[307.3827052922357, 279.4737669028152, 317.2824775426448, 308.2831475934922, 317.1823103827948, 337.0577174934995],"true", 3) )
...
(self.obj36,self.obj28,[349.56055185802165, 74.09238257128929, 373.00478409525414, 96.85116478948774, 389.4836186264952, 126.78512887279376],"true", 3) )
    # Connections for obj37 (graphObject_: Obj17) of type achieve
    self.drawConnections(
(self.obj37,self.obj31,[84.82626970983395, 362.1451725819457, 59.56099321994252, 385.43923105089016, 27.517274608438115, 401.38123386145287],"true", 3) )
    # Connections for obj38 (graphObject_: Obj19) of type achieve
    self.drawConnections(
(self.obj38,self.obj32,[317.1823103827948, 337.0577174934995, 317.0821432229448, 365.8322873935069, 306.9820366528357, 394.57204650284456],"true", 3) )
    # Connections for obj39 (graphObject_: Obj21) of type achieve
    self.drawConnections(
(self.obj39,self.obj32,[222.13756326537128, 342.3877754747146, 266.73847728657927, 363.97953710400697, 306.9820366528357, 394.57204650284456],"true", 3) )
    # Connections for obj40 (graphObject_: Obj23) of type require
    self.drawConnections(
(self.obj40,self.obj28,[340.414318871705, 195.90465454112874, 360.9395472052699, 168.3574950336234, 389.4836186264952, 146.78512887279376],"true", 3) )
    # Connections for obj41 (graphObject_: Obj24) of type require
    self.drawConnections(
(self.obj41,self.obj27,[244.92103503030822, 220.28528088040454, 216.79345843945703, 198.02060594998179, 194.87239892883093, 167.91506718112427],"true", 3) )
    # Connections for obj42 (graphObject_: Obj25) of type require
    self.drawConnections(
(self.obj42,self.obj27,[153.01081004957175, 221.9053316890469, 169.58431463977854, 192.45784848790922, 194.87239892883093, 167.91506718112427],"true", 3) )
    # Connections for obj43 (graphObject_: Obj26) of type require
    self.drawConnections(
(self.obj43,self.obj26,[67.38155529802249, 233.10874016814495, 40.486960156021546, 203.45178070451655, 21.0, 167.07716213116151],"true", 3) )

newfunction = omacsEx2_MDL
